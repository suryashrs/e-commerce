...
class ProductController {
    ...
    public function index(){
        ...
        if($num > 0){
            ...
            return array("status" => 200, "body" => []); // Return empty array instead of 404 for admin lists
        }
    }
        ...
        if(!empty($data['name']) && !empty($data['price'])){
            ...
            if(isset($files['image']) && $files['image']['error'] === 0){
                ...
                if(move_uploaded_file($files['image']['tmp_name'], $target_file)){
                    ...
                    // Automate Background Removal for specific categories
                    $cat = strtolower($this->product->category);
                    if ($cat == 'jacket' || $cat == 'shirt' || $cat == 't-shirt') {
                 ...
                 $this->product->try_on_image_url = $data['try_on_image_url'] ?? '';
            }
