...
import { Users, Search, Edit, Trash2, ShieldAlert, User, ShoppingBag, ShieldCheck, X, Filter, Clock, Mail, CheckCircle, AlertTriangle, XCircle } from "lucide-react";
...
const AdminUsers = () => {
    ...
    const handleDeleteUser = (user) => {
        setConfirmModal({
            ...
            message: `You are about to permanently delete "${user.name}" (${user.email}). This action cannot be undone.`,
    ...
    const executeDelete = async () => {
        ...
        try {
            ...
            if (res.data.status === 200) {
                ...
                addToast('User account permanently deleted.', 'success');
            } else {
                addToast(res.data.message || 'Failed to delete user.', 'error');
            }
        } catch (err) {
    ...
    const handleUpdateUser = async (e) => {
        ...
        try {
            ...
            if (res.data.status === 200) {
                ...
                addToast(res.data.message || 'Failed to update user.', 'error');
            }
        } catch (err) {
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-[10px] text-gray-500 flex items-center gap-1"><Mail size={10} /> {user.email}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                ...
                                                {user.role === 'seller' ? (
                                                    <div className={`flex items-center gap-2 text-[8px] font-black uppercase tracking-widest px-3 py-0.5 rounded-full w-fit border ${
                                                        ...
                                                        <div className={`w-1 h-1 rounded-full ${
                                                            ...
                                                            user.shop_status === 'pending' ? 'bg-amber-500' : 'bg-rose-500'
                                                        }`} />
                                                        {user.shop_status}
                                                    </div>
                                                ) : (
                            ...
                            <form onSubmit={handleUpdateUser} className="p-10 space-y-8">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 font-sans">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <select value={editingUser.shop_status} onChange={(e) => setEditingUser({...editingUser, shop_status: e.target.value})}
                                            className="w-full bg-gray-800/50 border border-transparent focus:border-indigo-500/50 rounded-2xl px-6 py-4 text-white font-bold text-sm focus:outline-none transition-all shadow-inner appearance-none">
